<template>
  <div id="app">
    <van-nav-bar title="交易所" left-arrow @click-left="onClickLeft" />
    ...
    <van-grid v-show="detailStep" :column-num="1" direction="horizontal">
      <van-grid-item icon="photo-o" text="返回" @click="goCat" />
    </van-grid>
    <van-grid v-show="catStep" :column-num="2" clickable direction="horizontal">
      <van-grid-item
        v-for="value in 20"
        :key="value"
        :icon="item_1"
      ...
      <van-grid-item v-for="value in 20" :key="value">
        <template #default>
          <van-cell center>
            ...
            <template #title>
              盗贼之衣
              <van-tag round color="#ffe1e1" text-color="#ad0000"
                >04:40</van-tag
              >
            </template>
            <template #right-icon>
